...
class cash(models.Model):
    ...
    choice_mode1=[
        ('Cash','Cash'),
        ('Online/Cheque','Online/Cheque'),
    ]

    date=models.DateTimeField(default=datetime.now)
    shop=models.CharField(max_length=50, null=False)
    amount=models.IntegerField(null=False)
    mode1=models.CharField(max_length=25,choices=choice_mode1, null=False, blank=True)
    agent=models.CharField(max_length=25,choices=choice_agent, null=False)
    deposit_status=models.CharField(max_length=5,choices=choice_deposit, null=False, blank=True)
    remarks=models.CharField(max_length=120, blank=True)

    def __str__(self):
        return f'SHOP: {self.shop}   STATUS: {self.deposit_status} DATE: {self.date}'
