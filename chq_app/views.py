...
from .models import master, order, replacement, new_order, boat_ticket, cash
from .forms import chq_form, order_form, replace_form, new_order_form, boat_ticket_form, cash_form
...
def cash_entry(request):
    form_cash_entry=cash_form()
    if request.method == 'POST':
        form_cash_entry=cash_form(request.POST)
        if form_cash_entry.is_valid():
            form_cash_entry.save()
            messages.info(request, 'Cash entry submitted successfully')
            return redirect('cash_entry')
    
    context={'form_cash_entry':form_cash_entry}
    return render(request, 'cash_entry.html',context)


def cash_report(request):
    if request.method == 'POST':
        agent=request.POST['collected_by']
        deposit_status=request.POST['status']

        cash_o=cash.objects.all()

        if agent:
            cash_o=cash_o.filter(agent=agent)
        if deposit_status:
            cash_o=cash_o.filter(deposit_status=deposit_status)

        context={'cash_o':cash_o}

        return render(request, 'cash_report.html',context)
    
    elif request.method == 'GET':
        return render(request, 'cash_report.html')
    else:
        return HttpResponse('An Exception Occureed')
    
def cash_entry_edit(request,id):
    if request.method == 'POST':
        edit_id=cash.objects.get(pk=id)
        edit_form=cash_form(request.POST, instance=edit_id)
        tttt=cash_form(request.GET, instance=edit_id)
        ...
        edit_id=cash.objects.get(pk=id)
        edit_form=cash_form(instance=edit_id)
        return render(request, 'cash_edit.html',{
            'edit_form':edit_form
        })
...
def cash_delete(request, id):
    if request.method == 'POST':
        cash_o=cash.objects.get(pk=id)
        cash_o.delete()
        return HttpResponseRedirect(reverse('cash_report'))
    ()
